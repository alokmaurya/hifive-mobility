...
function DriverCard({ driver, city, state }: { driver: Driver; city: string; state: string }) {
  ...
  return (
    <button
      ...
      className="w-full bg-blue-900 border border-blue-800 rounded-3xl p-4 text-left hover:border-white/30 transition-colors"
    >
      ...
      <div className="bg-blue-800 rounded-2xl h-28 flex items-center justify-center mb-3 relative overflow-hidden">
        {driver.carPhotoUrl ? (
          ...
          <div className="flex flex-col items-center gap-1">
            ...
            <span className="text-blue-400 text-xs">{driver.carBrand || "Car"} {driver.vehicleModel}</span>
          </div>
        )}
        {driver.vehiclePlate && (
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        <div className="bg-blue-800 rounded-xl px-3 py-2">
          <p className="text-blue-300 text-[10px] uppercase tracking-wide">Vehicle</p>
          <p className="text-white text-xs font-semibold mt-0.5">
            {driver.carBrand} {driver.vehicleModel}
          </p>
          <p className="text-blue-200 text-[10px]">{VEHICLE_LABEL[driver.vehicleType] ?? driver.vehicleType} · {FUEL_LABEL[driver.fuelType] ?? driver.fuelType}</p>
        </div>
        <div className="bg-blue-800 rounded-xl px-3 py-2">
          <p className="text-blue-300 text-[10px] uppercase tracking-wide">Capacity</p>
          ...
          <div className="flex items-center gap-1 mt-0.5">
            ...
            <span className="text-blue-200 text-[10px]">{driver.luggageCapacityBags} bags</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="ml-auto text-right">
          <span className="text-blue-300 text-[10px]">{driver.totalToursRun} trips</span>
        </div>
      </div>
    </button>
  );
      ...
      <div className="min-h-screen bg-blue-950 pb-20">
        ...
        <div className="bg-blue-900 border-b border-blue-800 px-4 pt-10 pb-4">
          <div className="max-w-md mx-auto">
            <button onClick={() => router.push("/traveller")} className="flex items-center gap-1 text-blue-200 hover:text-white text-sm mb-3">
            ...
            <form onSubmit={handleSearch} className="flex gap-2">
              <select
                ...
                className="flex-1 px-3 py-2.5 rounded-2xl border border-blue-700 bg-blue-800 text-white focus:outline-none focus:ring-2 focus:ring-white/50 text-sm"
              >
              ...
              <button type="submit" disabled={!selCity} className="px-4 py-2.5 bg-white text-blue-900 rounded-2xl hover:bg-blue-50 disabled:opacity-50 transition-colors">
        ...
        <div className="px-4 max-w-md mx-auto mt-4">
          ...
          {!city && !loading && (
            <p className="text-blue-300 text-sm text-center mt-8">Select a city above to find drivers</p>
          )}
          ...
          {!loading && drivers.length === 0 && city && (
            <div className="text-center mt-12">
              ...
              <p className="text-blue-200">No drivers with tours in {city}.</p>
              <p className="text-blue-400 text-sm mt-1">Try a different city or check back later.</p>
            </div>
          )}
